...
export const createAccessToken = (user: User): string => {
  ...
  const accessToken = jwt.sign(userData, process.env.JWT_SECRET_KEY || 'secret-key', { expiresIn: '30m' });
  return accessToken;
};
...
export const verifyAccessToken = (accessToken?: string): JwtVerifiedUser | null => {
  ...
  try {
    ...
    const verified = jwt.verify(
      accessToken, process.env.JWT_SECRET_KEY || DEFAUT_JWT_SECRET_KEY) as JwtVerifiedUser;
    return verified;
  } catch (err) {
