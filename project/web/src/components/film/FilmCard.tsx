...
import { 
  ...
  Image,        // 이미지 컴포넌트
  LinkBox,      // 클릭 가능한 박스를 만들기 위한 컴포넌트
...
interface FilmCardProps {
  film: FilmsQuery['films'][0]; // film은 GraphQL 쿼리에서 받은 영화 데이터를 타입으로 지정
}
    ...
    <LinkBox as="article" my={6}> 
      ...
      >
        ...
        <Box bg="gray" mt={-3} mx={-3} mb={2} pos="relative">
          ...
          <AspectRatio ratio={2 / 3}>
            <Image src={film.posterImg} /> {/* 영화 포스터 이미지 표시 */}
          </AspectRatio>
        </Box>
        ...
        <Stack>
          ...
          >
            {film.subtitle ? film.subtitle : <>&nbsp;</>} {/* 부제목이 있으면 부제목을 표시, 없으면 공백 */}
          </Text>
        </Stack>
        ...
        <Stack spacing={0} fontSize="sm" mt={2}>
          {/* 개봉일과 러닝 타임 */}
          <Text as="time" dateTime={film.release} isTruncated color="gray.500">
            {`${film.release}  .  ${film.runningTime}분`} {/* 개봉일과 러닝 타임을 표시 */}
          </Text>
          {/* 감독 이름 */}
          <Text isTruncated>{film.director.name}</Text> {/* 감독 이름을 표시 */}
        </Stack>
      </Box>
    </LinkBox>
  )
