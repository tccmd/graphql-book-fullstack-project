...
function Film(): React.ReactElement {
  ...
  const handleCutSelect = (cutId: number) => {
    console.log("이전 Cut ID:", selectedCutId);
    setSelectedCutId(cutId) // cut id
    onOpen() // modal open
  }

  useEffect(() => {
    console.log("selectedCutId가 변경되었습니다:", selectedCutId);

    if (selectedCutId === undefined) {
      console.log("선택된 Cut ID가 없습니다.");
    } else {
      console.log("선택된 Cut ID:", selectedCutId);
    }
  }, [selectedCutId])
